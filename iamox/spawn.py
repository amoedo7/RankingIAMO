...
def birth(number: int, born_at: str) -> dict[str, Any]:
    return {
        "id": f"iamo{number}",
        "name": f"IAMO{number}",
        "number": number,
        ...
        "runtime_generation": "0.3.0-individual",
        "origin": "DesarrollAMO",
        "creator": "AMO",
    }


def spawn(count: int = 1) -> list[dict[str, Any]]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create new IAMOX identities; runtime materializes their individual brain/world.")
    parser.add_argument("--count", type=int, default=1)
    args = parser.parse_args()
    created = spawn(args.count)
